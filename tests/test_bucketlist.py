...
class TestBucketList(unittest.TestCase):
    ...
    def test_bucketlist_is_created(self):
        ...
        self.assertTrue(isinstance(self.new_bucketlist, BucketList))
    ...
    def test_bucketlist_items_is_list(self):
        ...
        self.assertTrue(isinstance(self.new_bucketlist.items, list))
    ...
    def test_add_bucketlist_item(self):
        ...
        self.assertEqual(add_item[1], "ItemName added successfully")
        # ensure that it is indeed in the list of items
        self.assertIn(new_item.name, self.new_bucketlist.items)
        # check and ascertain that a bucketlist_item object is created
        self.assertTrue(isinstance(new_item[0], BucketListItem))
    ...
    def test_update_bucketlist_item_details(self):
        ...
        self.assertTrue(isinstance(new_item, BucketListItem))
        # unpark the details
        ...
        self.assertEqual((new_item.name, new_item.category, new_item.details), ("ChangedName", "ChangedCategory", "ChangedDescription"))
    ...
    def test_remove_bucketlist_item(self):
        ...
        self.assertEqual(remove_item[1], "ItemName removed successfully")
        # ensure that the item is no longer in the list
        self.assertNotIn(self.new_item_details[0], self.new_bucketlist.items)
        # ensure that the item details are indeed deleted from the object instance
        self.assertEqual(new_item.name, None)
    ...
    def test_deleting_non_existing_bucketlist_item(self):
        ...
        remove_item = self.new_bucketlist.remove_item(self.new_item_details[0])
        self.assertEqual(remove_item, "{} does not exist.".format(self.new_item_details[0]))
        # assert that we're raising exceptions
        self.assertTrue(isinstance(remove_item, Exception))
